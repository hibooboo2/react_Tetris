...
var GameBox = React.createClass({displayName: 'GameBox',
    ...
    drawPiece: function(aPiece) {
        return React.createElement(TetrisPiece, {piece: aPiece})
    },
    render: function() {
        ...
        var drawnBoard = React.createElement("div", {className: "cells"},
                            tempBoard.map(function(row) {
                                return row.map(function(cell) {
                                    return React.createElement("div", {className: "TetrisCell "+cell.getType(), style: positionCell(cell)});
                            });
                            }),
                            ...
                            React.createElement("p", {className: this.state.gameState.score.nextLevelUp().linesFromTetris ? "enabled" : "disabled"}, "Go for a tetris to Levelup!")
                        );
        var drawnHeld = false
        if(this.state.gameState.heldPiece){
            drawnHeld = React.createElement(TetrisPiece, {piece: new this.state.gameState.pieceEngine.newPiece(this.state.gameState.heldPiece,0,0,6)});
        };
        return (
            React.createElement("div", {className: "GameBox"},
                drawnBoard,
                    ...
                    React.createElement("div", {className: this.state.closeGameoverScreen? "disabled" : "gameOver"},
                        ...
                        React.createElement("input", {onClick: this.closeGameoverScreen, className: "button", type: "button", value: "Close"})
                    ),
                    React.createElement("div", {className: this.state.paused? "pauseScreen" : "disabled"},

                        React.createElement("p", {onClick: this.state.gameState.started ? this.pause : this.play, className: "pauseLabel "+(this.state.paused ?"paused":"notPaused")}, this.state.paused ?"Play":"Pause"),
                        React.createElement("div", {className: "keyMappings"},
                        this.state.keyMapping.keys.readableLines().map(function(line){
                            return React.createElement("p", {className: "leftAlign"}, line)
                        })
                        ),
                        React.createElement("div", null
                        ),
                        React.createElement("div", {className: "Settings"},
                            React.createElement("input", {onClick: this.toggleAutoSave, className: this.state.gameState.settings.useAutoSave ? "off":"on", type: "button", value: "Auto Save"}),
                            React.createElement("input", {onClick: this.toggleGhost, className: this.state.gameState.settings.useGhost ? "off":"on", type: "button", value: "Ghost"}),
                            React.createElement("input", {onClick: this.togglePreview, className: this.state.gameState.settings.canPreview ? "off":"on", type: "button", value: "Preview"}),
                            React.createElement("input", {onClick: this.toggleHold, className: this.state.gameState.settings.canHold ? "off":"on", type: "button", value: "Hold"}),
                            React.createElement("input", {onClick: this.togglefullScreen, className: this.state.gameState.settings.fullScreen ? "off":"on", type: "button", value: "Fullscreen"}),
                            React.createElement("input", {onClick: this.toggleAnnouncer, className: this.state.gameState.settings.announcer ? "off":"on", type: "button", value: "Announcer"}),
                            React.createElement(TetrisSong, null)
                        ),
                        React.createElement("div", {className: "Issues"},
                        React.createElement("hr", null),
                        "Any New issues? ", React.createElement("a", {href: "https://github.com/hibooboo2/react_Tetris/issues", target: "_blank"}, "Add Them"),
                        React.createElement("hr", null),
                ...
                React.createElement("div", null,
                    React.createElement("div", {className: "previewBox "+(this.state.gameState.settings.canPreview ? "enabled":"disabled")},
                            "Preview:" + ' ' +

                            "Next Piece:",
                            React.createElement("div", {style: {position:"absolute",top:"0em"}},
                                React.createElement(TetrisPiece, {piece: this.state.gameState.pieceEngine.que.slice(0).reverse()[0]})
                            ),
                            React.createElement("div", {style: {position:"absolute",top:"8em"}},
                                React.createElement(TetrisPiece, {piece: this.state.gameState.pieceEngine.que.slice(0).reverse()[1]})
                            )
                    ),
                    React.createElement("div", {className: "heldBox "+(this.state.gameState.settings.canHold ? "enabled":"disabled")},
                            React.createElement("div", {style: {top: "0em",position: "absolute"}, className: "inline"}, "Currently Holding:"),
                            drawnHeld
                    )
                ),
                ...
                React.createElement("audio", {id: "GodSong", width: "0", height: "0", loop: "1", autoPlay: "1", muted: true},
                    React.createElement("source", {src: "http://hibooboo2.github.io/react_Tetris/audio/test.mp3", type: "audio/mp3"}),
                    "Your browser does not support the video tag."
                ),
                React.createElement(ChatBox, {piece: this.state.gameState.pieceEngine.que.slice(0).reverse()[3]})
            )
        )
    }
});
...
var TetrisPiece = React.createClass({displayName: 'TetrisPiece',
    getInitialState: function(){
    ...
    render: function() {
        ...
        var theCells = cells.map(function(cell){
            return (
                        React.createElement("div", {className: "TetrisCell "+cell.getType(),
                                    style: positionCell(cell)}
                        )
                    )
        });
        var theCells = React.createElement("div", {className: "TetrisPiece"},
                        theCells
                        );
        return theCells;
    }
        ...
        return
                (
                    React.createElement("div", {className: "ChatBox"},
                        ...
                        React.createElement("button", {id: "sendMessage"}, "Send Message")
                    )
                );
    }
...
var TetrisSong = React.createClass({displayName: 'TetrisSong',
    getInitialState: function(){
        return {
    ...
    render: function() {
        ...
        return (
                React.createElement("div", {className: "TetrisSong", style: {margin:"0.1em"}},
                    React.createElement("audio", {id: "TetrisSong", width: "200", height: "50", loop: "1", autoPlay: "1"},
                        React.createElement("source", {src: "audio/tetrisSong.mp4", type: "video/mp4"}),
                        "Your browser does not support the video tag."
                    ),
                        React.createElement("img", {onClick: this.togglePlay, height: "20px", width: "20px", src: !this.state.muted ? "https://cdn1.iconfinder.com/data/icons/material-audio-video/20/pause-circle-outline-128.png" : "https://cdn1.iconfinder.com/data/icons/material-audio-video/20/play-circle-fill-128.png", alt: "Play Pause Button"}),
                    React.createElement("img", {onClick: this.volumeDown, height: "20px", width: "20px", alt: "Volume Down", src: "https://cdn2.iconfinder.com/data/icons/freecns-cumulus/16/519648-152_VolumeDown-128.png"}),
                    React.createElement("img", {onClick: this.volumeUp, height: "20px", width: "20px", alt: "Volume Up", src: "https://cdn2.iconfinder.com/data/icons/freecns-cumulus/16/519649-153_VolumeUp-128.png"})
                )
                );
            }
});

var TetrisGame = React.createClass({displayName: 'TetrisGame',
    render: function() {
        return (
                    React.createElement("div", {className: "TetrisGame"},
                        React.createElement(GameBox, {gameState: this.props.gameState, keyMappings: this.props.keyMappings})
                    )
                );
        }
    });
...
React.render(
  React.createElement(TetrisGame, {gameState: new boardEngine(), keyMappings: theKeys}),
